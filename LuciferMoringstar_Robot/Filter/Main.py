...
async def filter(client, message):
    ...
    if 2 < len(message.text) < 100:    
        btn = []
        search = message.text
        mo_tech_yt = f"**🗂️ Title:** {search}\n**⭐ Rating:** {random.choice(RATING)}\n**📤By [Anime World](https://t.me/free_cartoons)**"
...
So, wait till it uploads"""
        ...
        buttons.append(
            [InlineKeyboardButton(text="MORE RESULTS ⏩",callback_data=f"next_0_{keyword}")]
        )    
        buttons.append(
...
So, wait till it uploads"""
        ...
        buttons.append(
            [InlineKeyboardButton(text="CLICK HERE FOR MORE RESULTS⏩",callback_data=f"next_0_{keyword}")]
        )    
        buttons.append(
            [InlineKeyboardButton(text=f"📃 Pages 1/{data['total']}",callback_data="pages")]
        )
